...
{
	// console.log('selectOptions', value, clear);
	var v = value;
	var vT = typeof(value);
	// handle arrays
	...
	var c = clear || false;
	// has to be a string or regular expression (object in IE, function in Firefox)
	if(vT != "string" && isNaN(v) && vT != "function" && vT != "object") return this;
		...
		function()
		{
			...
			for(var i = 0; i<oL; i++)
			{
				if(v.constructor == RegExp)
				{
					if(o[i].value.match(v))
					{
						$(o[i]).attr('selected', true);
						optionMatch = true;
					}
					else if(c)
					{
						$(o[i]).attr('selected', false);
					}
				}
				else
				{
					// console.log('set value?', o[i].value, v);
					if(o[i].value == v)
					{
						// console.log('set it!');
						$(o[i]).attr('selected', true);
